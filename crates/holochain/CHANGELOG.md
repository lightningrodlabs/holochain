...
- FIELD-TEST-ONLY: mDNS LAN discovery now rebinds when this node's local address set changes, so a conductor started while its network interface is down becomes LAN-dialable once the link comes up, instead of staying invisible to its neighbours for the life of the process.
