...
build-workspace-wasmer_sys-transport_reticulum:
	cargo build \
		--workspace \
		--locked \
		--all-targets \
...
# execute tests on all crates with wasmer compiler and reticulum transport
test-workspace-wasmer_sys-transport_reticulum:
	RUST_BACKTRACE=1 cargo nextest run \
		--workspace \
		--locked \
		--no-default-features \
